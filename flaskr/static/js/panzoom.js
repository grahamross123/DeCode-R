export const panzoom = (selector, options = {}) => {
  ...
  const bound = ["inner", "outer", "none"].includes(options.bound)
    ...
    : "inner";
  const wheel_step =
    options.wheel_step > 0.01 && options.wheel_step < 4
      ? options.wheel_step
      : 0.2;
  const scale_min =
    options.scale_min > 0.01 && options.scale_min < 20
      ? options.scale_min
      : 0.01;
  const scale_max =
  ...
  function do_zoom(elem, deltaScale, offsetX, offsetY) {
    const matrix = new WebKitCSSMatrix(
      getComputedStyle(elem).getPropertyValue("transform")
    );
    const {
      a: scaleX,
      b: skewY,
    ...
    if (bound == "inner") {
      ...
      posX_max =
        ...
        elem.offsetWidth -
        (elem.offsetWidth / 2) * (newScale - 1) -
        translateX;
      posY_max =
        elem.parentNode.offsetHeight -
        elem.offsetHeight -
        (elem.offsetHeight / 2) * (newScale - 1) -
        translateY;
  ...
  function handle_pointerdown(e) {
    ...
    if (bound == "inner") {
      ...
      posX_max =
        ...
        (e.target.offsetWidth / 2) * (scale - 1) -
        translateX;
      posY_max =
        e.target.parentNode.offsetHeight -
        e.target.offsetHeight -
        (e.target.offsetHeight / 2) * (scale - 1) -
        translateY;
    } else if (bound == "outer") {
  ...
  function handle_touchmove(e) {
    ...
    if (e.targetTouches.length == 2 && e.changedTouches.length == 2) {
      ...
      const offsetX1 =
        ((e.touches[1].clientX - x) / width) * e.target.offsetWidth;
      const offsetY1 =
        ((e.touches[1].clientY - y) / height) * e.target.offsetHeight;
